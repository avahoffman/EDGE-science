...
remove_outliers <-
  function(df,
      ...
      for (Trt_ in distinct(site_level_dat, Trt)$Trt) {
        ...
        for (category_ in distinct(trt_level_dat, category)$category) {
          ...
          if ((length(influential[influential]) / nrow(cat_level_dat)) <= outlier_prop_threshold) {
            d <-
              cat_level_dat[!(influential), ] # drop any where they are not influential
            dropped_d <-
              cat_level_dat[(influential), ] # Keep dropped values for double checking
            
            # Bind to new data frames
            dat <- rbind(dat, d)
            dropped_dat <- rbind(dropped_dat, dropped_d)
            
          } else {
            # Simply bind if no outliers
    ...
    # Write the data that's dropped for double checking
    write.csv(dropped_dat,
              "data/EDGE_biomass_DROPPED.csv",
              row.names = FALSE)

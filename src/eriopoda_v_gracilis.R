...
collect_sev_data <-
  function(ambient_composition, sum_across_years = TRUE) {
    ...
    if (sum_across_years) {
      full_dat <-
        c4_dat %>% group_by(Site, Block, Plot, Trt, category) %>% summarise(
          biomass = sum(biomass) / length(eri_grac_years)
        )
    }
    ...
    wide_dat <- dcast(full_dat, Site + Block + Plot + Trt ~ category, value.var="biomass")
    # Join total to wide format
    full_dat <- full_join(totals, wide_dat, by = c("Site", "Block", "Plot", "Trt"))
    ...
    if (ambient_composition) {
      ...
      BUDA <- compare_dat %>% filter(Site == "SGS") %>% filter(BUDA.x > 0) %>% select(-BOGR.x, -BOGR.y)
      BOGR <- compare_dat %>% filter(Site == "CHY") %>% filter(BOGR.x > 0) %>% select(-BUDA.x, -BUDA.y)
